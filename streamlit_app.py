...
def process_data(file):
    ...
    # 提取表头结构
    header_L1 = df.iloc[2].ffill().tolist()  # 修复:  使用 ffill() 代替 fillna(method='ffill')
    ...
    data['base_代理商'] = data['base_代理商'].ffill()  # 修复: 使用 ffill()
...
def generate_complex_image(agent_name, agent_data):
    ...
    # 构建完整表格内容（表头 + 数据）
    table_content = []
    row0 = [x[0] for x in headers_plot]  # 第一层表头
    row1 = [x[1] for x in headers_plot]  # 第二层表头
